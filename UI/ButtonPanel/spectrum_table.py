# UI/ButtonPanel/spectrum_table.py
...
class SpectrumTable(QTableView):
    ...
    def clear_table(self):
        """Очищает таблицу, устанавливая пустую модель данных."""
        empty_df = pd.DataFrame(columns=["Длина_волны", "Интенсивность"])
        self.spectrum_model = PandasModel(empty_df)
        self.setModel(self.spectrum_model)
        logger.debug("Таблица очищена")
    ...
    def update_table(self, dataframe):
        ...
        # Делаем столбцы растягиваемыми после обновления модели
        header = self.horizontalHeader()
        header.setSectionResizeMode(QHeaderView.ResizeMode.Stretch)